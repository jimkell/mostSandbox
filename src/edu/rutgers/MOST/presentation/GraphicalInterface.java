...
//		}						
	...
	public void updateKeggIdColumn(Vector<SBMLMetabolite> metabolites, int modeltrimStartIndex, int modeltrimEndIndex, int keggIdColumn) {
		...
		for (int i = 0; i < metabolites.size(); i++) {
			...
			// probably should eventually add a dropbox for this
			String abbr = metabolites.get(i).getMetaboliteAbbreviation();
			if (GraphicalInterfaceConstants.USE_METAB_NAME_FOR_MATCHING_KEGG_IDS) {
				abbr = metabolites.get(i).getMetaboliteName();
			}
...
//				file = new File(nodeInfo.getDatabaseName() + suffix);
	...
	public void updateReactionEquation(int rowIndex, int reactionId, String oldEquation, String newEquation) {
    	...
    	Map<Integer, SBMLMetabolite> idMetaboliteMap = new HashMap<Integer, SBMLMetabolite>();
		for (int i = 0; i < allMetabolites.size(); i++) {
			idMetaboliteMap.put(allMetabolites.get(i).getId(), allMetabolites.get(i));
		}
...
//							unprocessedEqun.getReactants().get(j).setMetaboliteName(idMetaboliteMap.get(metabId).getMetaboliteName());
//							unprocessedEqun.getReactants().get(j).setCompartment(idMetaboliteMap.get(metabId).getCompartment());
...
//							unprocessedEqun.getProducts().get(j).setMetaboliteName(idMetaboliteMap.get(metabId).getMetaboliteName());
//							unprocessedEqun.getProducts().get(j).setCompartment(idMetaboliteMap.get(metabId).getCompartment());
...
//				}
	...
	public void updateMetabolitesCompartmentValue(int id, String metabAbbrev, String oldValue, String newValue) {
		rewriteReactionEquationNames(id, metabAbbrev, oldValue, newValue);
	...
	HighlightPredicate nonEditablePredicate = new HighlightPredicate() {
		public boolean isHighlighted(Component renderer ,ComponentAdapter adapter) {
			if (adapter.column == GraphicalInterfaceConstants.REACTION_EQUN_NAMES_COLUMN ||
					!isRoot || analysisRunning || timerRunning || isVisualizing) {									
				return true;
			} 
			return false;
		}
...
//				}
	 ...
	 * @param compartment
	 */
	public void rewriteReactionEquationNames(int id, String metabAbbrev, String oldCompartment, String compartment) {
		MetaboliteFactory aFactory = new MetaboliteFactory("SBML");
		ArrayList<Integer> participatingReactions = aFactory.participatingReactions(metabAbbrev);			
		// rewrite reactions 
		for (int i = 0; i < participatingReactions.size(); i++) {
			...
			createReactionsIdRowMap();
			updateReactionsCellById(equn.equationNames, participatingReactions.get(i), GraphicalInterfaceConstants.REACTION_EQUN_NAMES_COLUMN);	
			updateCompartmentList(equn.getCompartmentList(), oldCompartment, compartment);
			updateCompartmentList(equn.getCompartmentReactantsList(), oldCompartment, compartment);
			updateCompartmentList(equn.getCompartmentProductsList(), oldCompartment, compartment);
		}
	}
	
	public void updateCompartmentList(ArrayList<String> list, String oldValue, String newValue) {
